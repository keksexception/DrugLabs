...
public class Handler_v1_15_R1 implements Versionhandler,HighVersionHandler{
	...
	public int sendZombiePacket(Player p, Location loc) {
		...
		PacketAPI_v1_15_R1.sendPacket(p, new PacketPlayOutSpawnEntityLiving(zombie));
	...
	public void sendGuardianParticles(Player p, Location loc) {
		...
		PacketAPI_v1_15_R1.sendPacket(p, packet);
	}

	@Override
	public boolean hasTag(ItemStack i, String tag) {
		return new NBTTagManager_v1_15_R1(i).hasTag(tag);
	}

	@Override
	public void sendVillagerParticles(Location loc, float a, float b, float c, float d, int count) {
		ParticleSystem_v1_15_R1.sendParticle(Particle.VILLAGER_HAPPY, loc, a, b, c, d, count);	
	...
	public ItemStack addTag(ItemStack i, String tag, boolean b) {
		return new NBTTagManager_v1_15_R1(i).addNBT(tag, b);
	}
	@Override
