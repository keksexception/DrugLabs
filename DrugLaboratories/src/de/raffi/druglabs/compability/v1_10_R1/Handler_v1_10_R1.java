...
public class Handler_v1_10_R1 implements Versionhandler{
	...
	public int sendZombiePacket(Player p, Location loc) {
		...
		PacketAPI_v1_10_R1.sendPacket(p, new PacketPlayOutSpawnEntityLiving(zombie));
	...
	public void sendDestroyPacket(Player p, int id) {
		PacketAPI_v1_10_R1.destroyEntity(p, id);
	}

	@Override
	public void sendHeartParticles(Player p, Location loc, float a, float b, float c, float d, int count) {
		ParticleSystem_v1_10_R1.sendParticleTo(EnumParticle.HEART, p, loc, a, b, c, d, count);	
	}

	@Override
	public void sendRedstoneParticles(Player p, Location loc, float a, float b, float c, float d, int count) {
		ParticleSystem_v1_10_R1.sendParticleTo(EnumParticle.REDSTONE, p, loc, a, b, c, d, count);	
	}
	...
	public void sendGuardianParticles(Player p, Location loc) {
		...
		PacketAPI_v1_10_R1.sendPacket(p, packet);
	}

	@Override
	public boolean hasTag(ItemStack i, String tag) {
		return new NBTTagManager_v1_10_R1(i).hasTag(tag);
	}

	@Override
	public void sendVillagerParticles(Location loc, float a, float b, float c, float d, int count) {
		ParticleSystem_v1_10_R1.sendParticle(EnumParticle.VILLAGER_HAPPY, loc, a, b, c, d, count);	
	...
	public ItemStack addTag(ItemStack i, String tag, boolean b) {
		return new NBTTagManager_v1_10_R1(i).addNBT(tag, b);
	}
	@Override
